...
class RFQService:
    ...
    def create_rfq(self, data: RFQCreate, org_id: int) -> RFQ:
        ...
        if org:
            # Handle both Enum instance and raw string from DB
            actual_type = getattr(org.org_type, 'value', org.org_type)
            if actual_type != OrgTypeEnum.customer.value:
    ...
    def select_quote(self, rfq_id: int, quote_id: int, selecting_org_id: int) -> Quote:
        ...
        org_actual_type = getattr(selecting_org.org_type, 'value', selecting_org.org_type) if selecting_org else None
        if not selecting_org or org_actual_type != OrgTypeEnum.customer.value:
            raise ForbiddenException(
