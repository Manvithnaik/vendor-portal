...
class OrderService:
    ...
    # ------------------------------------------------------------------
    def vendor_respond(
        self, order_id: int, data: VendorOrderResponse, vendor_user_id: int, vendor_org_id: int = None
    ) -> Order:
        order = self.repo.get(order_id)
        if not order:
            raise NotFoundException("Order")

        if vendor_org_id and order.manufacturer_org_id != vendor_org_id:
            raise UnauthorizedException("Unauthorized: This order does not belong to your organization.")
    ...
    # ------------------------------------------------------------------
    def update_status(
        self, order_id: int, data: OrderStatusUpdate, current_user_id: int, current_org_id: int = None, org_type_name: str = None
    ) -> Order:
        order = self.get_by_id(order_id, current_org_id, org_type_name)
    ...
    # Queries
    # ------------------------------------------------------------------
    def get_by_id(self, order_id: int, current_org_id: int = None, org_type_name: str = None) -> Order:
        order = self.repo.get(order_id)
        if not order:
            raise NotFoundException("Order")
