...
def get_order(order_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    svc = OrderService(db)
    order = svc.get_by_id(order_id, current_user.org_id, current_user.organization.org_type.name)
...
def vendor_respond_to_order(
    ...
    """
    svc = OrderService(db)
    order = svc.vendor_respond(order_id, data, current_user.id, current_user.org_id)
...
def update_order_status(
    ...
    svc = OrderService(db)
    order = svc.update_status(order_id, data, current_user.id, current_user.org_id, current_user.organization.org_type.name)
...
def get_order_history(order_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    svc = OrderService(db)
    history = svc.get_status_history(order_id, current_user.org_id, current_user.organization.org_type.name)
    data = [
        {
            "id": h.id,
