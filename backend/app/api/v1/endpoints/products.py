...
def list_products(
    ...
    # STRICT ISOLATION: Vendors can only list their own products
    if current_user.organization.org_type == OrgTypeEnum.manufacturer:
...
def update_product(product_id: int, data: ProductUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    svc = ProductService(db)
    product = svc.update(product_id, data, current_user.org_id)
...
def deactivate_product(product_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    svc = ProductService(db)
    svc.deactivate(product_id, current_user.org_id)
