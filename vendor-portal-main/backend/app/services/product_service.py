...
class ProductService:
    ...
    def create(self, data: ProductCreate, current_user_org_id: int) -> Product:
        # Override manufacturer_org_id to prevent spoofing
        data.manufacturer_org_id = current_user_org_id

        # Check for duplicate SKU within org
        existing = self.repo.get_by_sku(current_user_org_id, data.sku)
        if existing:
    ...
    def update(self, product_id: int, data: ProductUpdate, current_org_id: int = None) -> Product:
        product = self.get_by_id(product_id)
        if current_org_id and product.manufacturer_org_id != current_org_id:
            raise UnauthorizedException("You do not have permission to modify this product.")
    ...
    def deactivate(self, product_id: int, current_org_id: int = None) -> Product:
