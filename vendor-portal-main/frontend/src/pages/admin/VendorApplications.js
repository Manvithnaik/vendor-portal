                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      {app.verification_status !== 'verified' && (
                        <button onClick={() => handleActionClick(app.id, 'approved')} className="p-1.5 rounded-lg hover:bg-green-50 text-green-600" title="Approve">
                          <CheckCircle size={15} />
                        </button>
                      )}
                      {app.verification_status !== 'rejected' && (
                        <button onClick={() => handleActionClick(app.id, 'rejected')} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500" title="Reject">
                          <XCircle size={15} />
                        </button>
                      )}
                      {app.verification_status !== 'pending' && (
                        <button onClick={() => handleActionClick(app.id, 'pending')} className="p-1.5 rounded-lg hover:bg-orange-50 text-orange-500" title="Reset to Pending">
                          <RotateCcw size={15} />
                        </button>
                      )}
                    </div>
