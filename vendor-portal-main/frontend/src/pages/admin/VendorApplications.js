      ...
      <div className="space-y-6 text-sm">
        ...
        {(certs.length > 0 || extraDocs.length > 0) && (
          <div>
            ...
            <div className="space-y-3">
              {[...certs.filter(c => c.document_url).map(c => ({
                name: `${c.issued_by ? c.issued_by + ' — ' : ''}${c.document_url.split('/').pop()}`,
                url: c.document_url,
              })), ...extraDocs].map((doc, i) => (
                <a
                  key={i}
        ...
        <div className="pt-4 border-t border-surface-200 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {!isApproved && (
              <button
                onClick={() => startAction(app, 'approved')}
            ...
            {!isRejected && (
              <button
                onClick={() => startAction(app, 'rejected')}
            ...
            <button
              onClick={() => startAction(app, 'resubmit')}
